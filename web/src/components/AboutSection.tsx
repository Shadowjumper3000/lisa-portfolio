...
export default function AboutSection({ backgroundUrl }: AboutSectionProps) {
  ...
  return (
    <section className="relative min-h-screen w-full overflow-hidden flex">
      ...
      {/* Right 1/3 overlay with bright background - mobile has semi-transparent overlay */}
      <div className="w-full md:w-1/3 bg-background/40 md:bg-secondary/50 flex items-center justify-center relative z-10 py-20 backdrop-blur-sm md:backdrop-blur-none">
        <motion.div
          initial={{ opacity: 0, x: 40 }}
          whileInView={{ opacity: 1, x: 0 }}
          viewport={{ once: true, amount: 0.4 }}
          transition={{ duration: 0.8, ease: "easeOut" }}
          className="px-8 md:px-12 lg:px-16 max-w-prose text-left"
        >
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-serif font-bold text-foreground drop-shadow-lg">
            About Me
          </h2>
          <p className="mt-6 text-base md:text-lg text-slate-900 md:text-foreground leading-relaxed drop-shadow-md">
